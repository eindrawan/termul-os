    ...
    var modal = api.ui.modal({
      ...
      buttons: [
        ...
        },
        {
          ...
          onClick: function (m) {
            var remotePath = m.el
              .querySelector("#fv-remote-path-input")
              .value.trim();
  ...
  async function openFile(path, source) {
    ...
    try {
      ...
      if (source === SOURCE_LOCAL) {
        ...
        // Read remote binary file as base64 via SFTP or FTP
        var result;
        if (isFtp()) {
          result = await window.termulAPI.ftp.readFileBase64(
            api.connectionId,
            path,
          );
        } else {
          result = await window.termulAPI.ssh.sftpReadFileBase64(
            api.connectionId,
            path,
          );
        }
      ...
      var mimeType =
        fileType === TYPE_IMAGE ? getMimeType(path) : "application/pdf";
  ...
  function renderTabs() {
    ...
    for (var i = 0; i < openFiles.length; i++) {
      ...
      html +=
        ...
        '">';
      html +=
        '<div class="fv-tab-icon ' + file.source + '">' + sourceIcon + "</div>";
      html += '<span class="fv-tab-name">' + escapeHtml(file.name) + "</span>";
      html +=
        '<button class="fv-tab-close" title="Close">' +
        ICON_CLOSE +
        "</button>";
      html += "</div>";
    }
  ...
  function updateStatusBar() {
    ...
    els.statusType.textContent =
      file.type === TYPE_IMAGE
        ? "Image (" + getFileExtension(file.path).toUpperCase() + ")"
        : "PDF Document";
    els.statusSource.textContent =
